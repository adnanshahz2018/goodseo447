...
class amazon:
    ...
    def scrape(self):
        ...
        div = soup.find_all('div', attrs={'class': 'a-section a-spacing-medium'})
        our_div = div[0]
    ...
    def get_info(self, browser, title, url):
        browser.get(url)
        source = browser.page_source
        soup = BeautifulSoup(source, features='lxml')
        ...
        self.write_to_excel(title, url, price, asin)
    ...
    def create_excel_file(self):
        ...
        ws = wb['data']
        ws.append(['Title','Web-url', 'Price', 'ASIN'])
        wb.save(self.filename)
        wb.close()

    def write_to_excel(self, title, web_url, price, asin):
        wb = op.load_workbook(self.filename, False)
        ws = wb['data']
        ws.append([title, web_url, price, asin])
        wb.save(self.filename)
        wb.close()
